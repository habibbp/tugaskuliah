...
class IndexController extends BaseController{
	...
	private function login_user($username , $password_text, $rememberme = false){
		...
		$db->where("name_pelanggan", $username);
		...
		if(!empty($user)){
			...
			if(password_verify($password_text,$password_hash)){
				...
				if($rememberme == true){
					...
					$db->where("id_pelanggan", $user['id_pelanggan']);
	...
	function register($formdata = null){
		if($formdata){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename;
			$fields = $this->fields = array("name_pelanggan","email","photo","password"); //registration fields
			...
			if($this->validated()){
				...
				if($rec_id){
					$this->login_user($modeldata['name_pelanggan'] , $password_text);
					return;
				}
				else{
					$this->set_page_error();
